	...
	if (req.readyState==4)
	{
		...
		{			
			...
			if (req.responseText=="OK")
			{
				...
				armarCategoria(document.getElementById('categoria').value);
			}
			else
			{
...
function armarCategoria(id)
{
	req=getXMLHttpRequest();
	req.onreadystatechange=process_armarCategoria;
	req.open("GET","ajax_armarCategoria.php?id="+id,true);
	req.send(null);
}
function process_armarCategoria()
{
	if (req.readyState==4)
	{
		if (req.status==200)
			document.getElementById("contenido_categoria").innerHTML=req.responseText;
		else
			window.alert("Ha ocurrido un problema");      
	}
}
...
{
	req=getXMLHttpRequest();
	req.onreadystatechange=process_resetCategoria;
	req.open("GET","ajax_resetCategoria.php",true);
	req.send(null);
}
function process_resetCategoria()
{
	if (req.readyState==4)
	{
		if (req.status==200)

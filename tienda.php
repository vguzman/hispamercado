  ...
  <tr>
	...
	if (isset($_GET['cat'])==false)
	{
		...
		for ($i=0;$i<mysql_num_rows($query);$i++)
		{
			...
			$aux.=" AND id_usuario=".$user_tienda->id;
			...
			if (mysql_result($query2,0,0)!="0")
				echo '<div style="margin-bottom:5px; ">&raquo; <a href="'.trim($tienda->nick).'/'.$cate->id.'/" class="LinkFuncionalidad12">'.$cate->nombre.' ('.mysql_result($query2,0,0).')</a></div>';
	...
	else
	{
		...
		if ($cate1->esPadre()==false)	
			$url=trim($tienda->nick)."/".$cate1->padre()."/";
		else
			$url=trim($tienda->nick)."/";

<?
	...
	$query=operacionSQL("SELECT id_fb,id FROM Anuncio_Comentario WHERE id_anuncio=".$anuncio->id);
	for ($j=0;$j<mysql_num_rows($query);$j++)
	{
		...
		if ($presente==false)
		{
			operacionSQL("DELETE FROM Anuncio_Comentario WHERE id_fb='".$id_actual."'");
		
			//ELIMINO TODOS AQUELLOS CUYO PADRE ES EL QUE ESTOY ELIMINANDO
			operacionSQL("DELETE FROM Anuncio_Comentario WHERE id_comentario_padre=".$id_interno);
		}
